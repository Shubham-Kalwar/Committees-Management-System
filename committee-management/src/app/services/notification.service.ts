...
export class NotificationService {
  ...
  /**
   * Show a transient toast notification that auto-dismisses after the given
   * duration. Useful for displaying quick feedback (e.g. HTTP error messages)
   * without permanently polluting the notification drawer.
   */
  showToast(message: string, level: NotificationLevel = 'error', durationMs = 6000): void {
    ...
    setTimeout(() => this.remove(notification.id), durationMs);
  }

  private getToastTitle(level: NotificationLevel): string {
    if (level === 'success') {
      return 'Success';
    }
    if (level === 'warning') {
