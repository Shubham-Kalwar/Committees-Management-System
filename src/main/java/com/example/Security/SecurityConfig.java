    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http.csrf(csrf -> csrf.disable())
            ...
            .authorizeHttpRequests(authz -> authz
                ...
                .requestMatchers(HttpMethod.PUT, "/api/users/onboard").authenticated()
                ...
                .requestMatchers(HttpMethod.POST, "/api/committee/apply").hasRole("STUDENT")
                .requestMatchers(HttpMethod.GET, "/api/committee/my").authenticated()
                .requestMatchers(HttpMethod.GET, "/api/committee/applications/**").hasAnyRole("ADMIN", "FACULTY")
                .requestMatchers(HttpMethod.PUT, "/api/committee/approve/**").hasAnyRole("ADMIN", "FACULTY")
                .requestMatchers(HttpMethod.PUT, "/api/committee/reject/**").hasAnyRole("ADMIN", "FACULTY")
                // Committee CRUD — Admin only
                .requestMatchers(HttpMethod.POST, "/api/committees").hasRole("ADMIN")
                .requestMatchers(HttpMethod.PUT, "/api/committees/**").hasRole("ADMIN")
                .requestMatchers(HttpMethod.PATCH, "/api/committees/**").hasRole("ADMIN")
                .requestMatchers(HttpMethod.DELETE, "/api/committees/**").hasRole("ADMIN")
                .requestMatchers(HttpMethod.GET, "/api/committees/**").authenticated()
                ...
                // Event Media — upload/delete restricted to ADMIN/FACULTY, GET for all authenticated
                .requestMatchers("/uploads/**").permitAll()
                .requestMatchers(HttpMethod.POST, "/api/event-media/upload").hasAnyRole("ADMIN", "FACULTY")
                .requestMatchers(HttpMethod.POST, "/api/media/upload").hasAnyRole("ADMIN", "FACULTY")
                .requestMatchers(HttpMethod.DELETE, "/api/event-media/**").hasAnyRole("ADMIN", "FACULTY")
                .requestMatchers(HttpMethod.GET, "/api/event-media/**").authenticated()
                // Event Feedback — submit for all authenticated, analytics/view for all authenticated
                .requestMatchers(HttpMethod.POST, "/api/event-feedback").authenticated()
                .requestMatchers(HttpMethod.GET, "/api/event-feedback/**").authenticated()
                .requestMatchers(HttpMethod.DELETE, "/api/event-feedback/**").hasAnyRole("ADMIN", "FACULTY")
                // Catch-all rules
